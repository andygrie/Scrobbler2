...
export class AlbumService{
    ...
    getReleasesFromMusicBrainzByArtistAndRelease(artist : string, release : string) : Promise<any>{
        return this.http.get('http://musicbrainz.org/ws/2/release?inc=discid&query=artist:"' + artist + '" AND release:"' + release + '"')
            .toPromise()
            .then(data => data.json() as any)
            .catch(this.handleError);
    }
    getRecordingsFromMusicBrainzByRelease(mbid : string) : Promise<any>{
        ...
        return this.http.get('http://musicbrainz.org/ws/2/release?release-group=' + mbid  + '&inc=recordings')
            .toPromise()
            .then(data => data.json() as any)
            .catch(this.handleError);
    }
    ...
    getOptimalReleaseFromArray(releases : Array<any>) : any
    {
        ...
        for(let rel of releases){
            let helperObject = helperArray.find(obj => obj.trackCount == rel.media[0].tracks.length);
            if(helperObject != undefined)
                helperObject.count += 1;
            else{
        ...
        return releases.find(rel => rel.media[0].tracks.length == helperArray[0].trackCount);
    }
