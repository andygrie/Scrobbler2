        ...
        this.userService.getTopArtists(200, 1).then(data => {
            ...
            for(let i = 0; i < data.length; i++){
                this.artistService.getTopTags(data[i].name).then(tagData =>{
                    let tag = this.topTags.find(tag => tag.tagname.toLowerCase() === tagData[0].name.toLowerCase());
                    if(tag == undefined){
                        this.topTags.push({'tagname' : this.toTitleCase(tagData[0].name), 'count' : parseInt(data[i].playcount)});
                    }
                    else
                        tag.count += parseInt(data[i].playcount);
