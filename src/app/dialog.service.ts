...
export class DialogService {
    ...
    public confirmAlbumsOfArtistDialog(title: string, friends: string[], startDate : number, endDate : number): Observable<boolean> {
        let dialogRef: MdDialogRef<ConfirmDialogAlbumsOfArtist>;
        dialogRef = this.dialog.open(ConfirmDialogAlbumsOfArtist, { width : '75%', height : '75%'});
    ...
    public confirmTrackinfoDialog(title: string, artist: string, album: string): Observable<boolean> {
        let dialogRef: MdDialogRef<ConfirmDialogTrackInfo>;
        dialogRef = this.dialog.open(ConfirmDialogTrackInfo, { width : '75%', height : '75%'});
        dialogRef.componentInstance.title = title;
    ...
    public confirmNewScrobbleDialog() : Observable<boolean>{
        ...
        dialogRef = this.dialog.open(ConfirmDialogNewScrobble, { width : '75%', height : '75%'});
        return dialogRef.afterClosed();
    }
    public confirmNewScrobbleDialogWithDateAfter(date : number) : Observable<any>{
        ...
        dialogRef.componentInstance.datetimeAfterText = new Date(date).toLocaleDateString() + ' ' + new Date(date).toLocaleTimeString();
        return dialogRef.afterClosed();
    }
}