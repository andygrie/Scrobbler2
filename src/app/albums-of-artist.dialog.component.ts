...
export class ConfirmDialogAlbumsOfArtist implements OnInit{
    ...
    constructor(public dialogRef: MdDialogRef<ConfirmDialogAlbumsOfArtist>, private userService : UserService) {
    }
    ...
    addAlbumStatistics(friend : string, page : number) : void{
        ...
        this.userService.getArtistTracks(page, this.title, this.startDate, this.endDate, friend).then(data => {
            console.log(data);
            for(let track of data){
                if(track.album['#text'] != ''){
                    let existingAlbum = this.albums.find(album => album.name == track.album['#text']);
                    if(existingAlbum == undefined){
                        this.albums.push({name : track.album['#text'], playcount : 1, imageURL : track.image[3]['#text'], listeners : [friend]});
                    }
                    else{
                        existingAlbum.playcount++;
                        if(existingAlbum.listeners.find(listener => listener == friend) == undefined)
                            existingAlbum.listeners.push(friend);
                    }
                }
                else{
                    let existingAlbum = this.albums.find(album => album.name == 'UNKNOWN ALBUM');
                    if(existingAlbum == undefined){
                        this.albums.push({name : 'UNKNOWN ALBUM', playcount : 1, imageURL : '', listeners : [friend]});
                    }
                    else{
                        existingAlbum.playcount++;
                        if(existingAlbum.listeners.find(listener => listener == friend) == undefined)
            ...
            if(data.length == 200)
                this.addAlbumStatistics(friend, page + 1);
            else{
                this.friendsDone++;
            }
            if(this.friendsDone == this.friends.length)
                this.albums.sort(function(a,b){ return b.playcount - a.playcount;});
