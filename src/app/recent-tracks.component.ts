...
export class RecentTracksComponent implements OnInit {
    ...
    track(event) {
        if (this.loadHelper == true && (window.innerHeight + window.scrollY) >= document.body.offsetHeight && !this.anyTrackSelected()) {
    ...
    conventionsWarning(track : any, index : any) : boolean{
        ...
        for(let i = 0; i < conspicuousStrings.length && warning === false; i++){
            if(track.album['#text'].indexOf(conspicuousStrings[i]) === -1 && 
    ...
    getAlbumsForSpecificTrack(track : any) : void{
        ...
        if(track.selected && !alreadyTracksSelected){
            ...
            this.loadReleaseGroups(0,encodeURIComponent(track.artist['#text']), track.name);
        }
    ...
    loadReleaseGroups(offset : number, artistName : string, track : string) : void{
        this.artistService.getAlbumInfoFromMusicBrainzByArtistName(artistName, offset).then(releases => {
    ...
    findValidAlbumsForUpdate(artistName : string, track : string){
            this.artistService.getRecordingsAndReleasesFromMusicBrainzByArtistNameAndRecording(artistName, track, 0).then(recordingResponse => {
    ...
    validateAlbumsForUpdate(helperArray) : void{
        helperArray.forEach((helper: any) => {
            let correctAlbum = this.albumsForUpdate.find(album => album.title === helper)
    ...
    scrobbleSelectedTracks() : void {
        let selectedTracks = this.recentTracks.filter(track => track.selected == true);
        
        for(let track of selectedTracks){

            let album;
            if(this.selectionType == "select")
                album = this.newAlbum.title;
            else
                album = this.newAlbumWritten;

            this.trackService.scrobbleTrack(track.artist['#text'], track.name, album, parseInt(track.date.uts) + 60).then(data => {
                this.userService.getTracksBetweenBoundaries(1, 1, parseInt(data.scrobbles.scrobble.timestamp) - 1, parseInt(data.scrobbles.scrobble.timestamp) + 1, this.localStorageService.get('name').toString())
                    .then(correctedScrobbles => {
                        ...
                        track.streamable = correctedScrobbles.recenttracks.track[0].streamable;
    ...
    anyTrackSelected() : boolean {
        return this.recentTracks.filter(track => track.selected == true).length != 0;
    }
    ...
    public openDialogTrackinfo(artistName : string, trackName : string, albumName : string) {
        this.dialogService
    ...
    encodeAlbumLinkURIComponent(artist : string, album : string) : string{
        ...
        return 'album/' + encodeURIComponent(album) + '/' + encodeURIComponent(artist);
    }
    openDialogNewScrobbleBefore(date) : void{
        ...
        .subscribe(res => {
          ...
          this.userService.getTracksBetweenBoundaries(200, 1, parseInt(res.startDate) - 1, parseInt(res.endDate) + 1, this.localStorageService.get('name').toString())
                .then(scrobbles => {
                    for(let i = 0; i < scrobbles.recenttracks.track.length; i++){
                        if(scrobbles.recenttracks.track[i].date != undefined){
                            scrobbles.recenttracks.track[i].date['#text'] = new Date(parseInt(scrobbles.recenttracks.track[i].date.uts) * 1000);
