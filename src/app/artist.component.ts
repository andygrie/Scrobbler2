...
export class ArtistComponent implements OnInit {
    ...
    getArtistInfo() : void{
        this.artistService.getInfo(this.artistName, this.localStorageService.get('name').toString()).then(data => {
    ...
    addUserAlbumStatistics() : void{
        ...
        for(var track of this.artistTracks){
            if(track.album['#text'] != ''){
                let existingStatistic = this.userAlbumStatistics.find(album => album.name == track.album['#text']);
                if(existingStatistic == undefined)
                    this.userAlbumStatistics.push({name : track.album['#text'], imageURL : track.image[3]['#text'], playcount : 1});
                else
                    existingStatistic.playcount++;
            }
            else{
                let existingStatistic = this.userAlbumStatistics.find(album => album.name == 'UNKNOWN ALBUM');
                if(existingStatistic == undefined)
                    this.userAlbumStatistics.push({name : 'UNKNOWN ALBUM', imageURL : track.image[3]['#text'], playcount : 1});
                else
                    existingStatistic.playcount++;
            }
        }
        this.userAlbumStatistics.sort(function(a,b){ return b.playcount - a.playcount;});
    }
    addGlobalAlbumStatistics(globalAlbums : any[]) : void{
        ...
        for(var album of globalAlbums){
            if(album.image[3]['#text'] != ''){
                this.globalAlbumStatistics.push({name : album.name, imageURL : album.image[3]['#text'], playcount : album.playcount});
            }
        }
        this.globalAlbumStatistics.sort(function(a,b){ return b.playcount - a.playcount;});
        ...
        for(var album of this.globalAlbumStatistics){
            ...
            this.albumService.getInfo(name, this.artistName, '').then(data => {
                if(data.album && data.album.tracks.track.length == 0){
                    this.globalAlbumStatistics = this.globalAlbumStatistics.filter(existingAlbum => existingAlbum.name != name);
                }
            }, this.showErrorMessage);
        }
    }
