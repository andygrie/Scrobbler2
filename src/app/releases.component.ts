...
export class ReleasesComponent implements OnInit {
    ...
    getArtists(page : number, artists : any[]){
      this.userService.getTopArtists(200,page).then(retArtists => {
        ...
        console.log(artists);
        if(retArtists.length < 200)
          this.getAlbumInfo(artists);
        else
          this.getArtists(page + 1, artists);
    ...
    getAlbumInfo(artists) : void{
        if(artists[0].mbid != undefined && artists[0].mbid != ''){
          ...
          this.delay(700).then(result =>{
              this.artistService.getAlbumInfoFromMusicBrainzByArtistId(artists[0].mbid).then(info => {
                console.log(info);
                this.artists.push(info);
                for(let rg of info['release-groups'])
                  rg.artist = artists[0].name;
