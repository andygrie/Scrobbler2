...
export class ConfirmDialogNewScrobble implements OnInit {
  ...
  onSubmit() {
    ...
    if (this.datetime != undefined) millis = new Date(this.datetime).getTime();
    else if (this.datetimeAfter != undefined) {
      if (this.wholeAlbum)
        this.datetimeAfter =
          this.datetimeAfter -
          this.lastSelectedTrackMillis +
          this.tracksForUpdate[0].length;
    ...
    if (this.wholeAlbum) this.onSubmitWholeAlbum(0, seconds, correctAlbum);
      ...
      this.trackService
        .scrobbleTrack(this.artist, correctTrack, correctAlbum, seconds)
        .then(data => {
          console.log(data);
