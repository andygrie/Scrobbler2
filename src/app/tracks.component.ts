...
export class TracksComponent implements OnInit {
    ...
    track(event) {
        if (this.loadHelper == true && (window.innerHeight + window.scrollY) >= document.body.offsetHeight) {
            ...
            this.userService.getTopTracks(200, this.currentPage, this.period).then(data => {
    ...
    ngOnInit(): void {
        this.isRequesting = true;
        this.userService.getTopTracks(this.limit, this.currentPage, 'overall').then(data => {
          this.topTracks = data;
          this.stopRefreshing();
          this.loadHelper = true;
      }, this.showErrorMessage);
    ...
    getWidthPercentage(track : any) : number{
        return parseInt(track.playcount) / parseInt(this.topTracks[0].playcount) * 100 / 2 + 50;
    }
    changeTimeRange(period : string) : void{
        this.currentPage = 1;
        this.isRequesting = true;
        this.period = period;
        this.userService.getTopTracks(this.limit, this.currentPage, this.period).then(data => {
          this.topTracks = data;
