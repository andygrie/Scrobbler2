...
export class FriendsComponent implements OnInit {
    ...
    getRecentTracksOfFriend(friend : any){
        ...
        this.userService.getWeeklyArtistChart(this.startDate, this.endDate, friend.name).then(data => {
    ...
    addArtistStatistics(statistics : any[], friend : string) : void{
        for(let artistStatistic of statistics){
            let existingStatistics = this.artists.find(artist => artist.artist == artistStatistic.name);
            if(existingStatistics == undefined){
                this.artists.push({artist : artistStatistic.name, playcount : parseInt(artistStatistic.playcount), imageURL : '', listeners : [friend]});
    ...
    public openDialog(artist : any) {
        this.dialogService
        .confirmAlbumsOfArtistDialog(artist.artist, artist.listeners, this.startDate, this.endDate)
        .subscribe(res => {
          if(res)
            this.router.navigate(['/artist', artist.artist]);
        });
    }
